...
#include "csc300222fall/assignment5/tokenscanner.h"
#include "csc300222fall/assignment5/stringmap.h"
#include <cctype>
#include <iostream>
#include <sstream>
#include <stack>
...
void StringMap::rehash(int nBuckets) {
   if (nBuckets < size()) {return;}
   KeyValuePair *temp = buckets;
   int cnt = count;
   int n = this->nBuckets;
   ...
   for(int i = 0; i < n; i++) {
      if (temp[i].occupied) {
         int idx = insertKey(temp[i].key);
         this->buckets[idx].value = temp[i].value;
...
int StringMap::findKey(const string & key) const {
   ...
   if (!buckets[index].occupied) {return -1;}
...
int StringMap::insertKey(const string & key) {
   ...
   while (buckets[index].occupied) {
      index = (index + 1) % nBuckets;
      //when index out of range, find from begin of buckets
   }
