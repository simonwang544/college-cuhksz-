...
void PriorityQueue<ValueType>::enqueue(ValueType value, double priority) {
   ...
   if (count == 0) {
      ...
      Cell *cp = head;
      while(cp->priority < priority && cp->link != NULL) {
         cp = cp->link;
      }     
      if (cp->priority > priority) {
         Cell *find_cell = new Cell;
         for (Cell *temp = head; temp!=NULL; temp = temp->link) {
             if (temp->link == cp) {find_cell = temp;}
         }
         newCell->link = find_cell->link;
         find_cell->link = newCell;
         count ++;
         return;
      }
      else {
...
ValueType PriorityQueue<ValueType>::dequeue() {
   if (count == 0) {error("Dequeue from an empty set.");}
   else{
     ValueType temp = head->data;
     head = head->link;
     count -= 1;
     return temp;
   }
...
/*template <typename ValueType>
void PriorityQueue<ValueType>::printQueue(const PriorityQueue<ValueType> &src) {
     for (Cell *temp = src->head; temp != NULL; temp = temp->link) {
        cout << temp->data << endl;
     }
}*/
